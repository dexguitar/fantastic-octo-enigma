...
const startService = async () => {
  try {
    ...
    await initConsumer(consumer, [config.topics.textProcessing], handleMessage);
    logger.info('Text service started and listening for messages');

    app.listen(config.textService.port, config.textService.host, () => {
      logger.info(
        `Text service health check available at http://${config.textService.host}:${config.textService.port}/health`
      );
    });
  } catch (error) {
